...
class PixelOPTConfig(OPTConfig):
    model_type = "PixelOPT"
...
class PixelOPTAttention(OPTAttention):
...
class PixelOPTDecoderLayer(OPTDecoderLayer):
    def __init__(self, config):
        super().__init__(config)

        if config.add_cross_attention:
            self.encoder_attn = PixelOPTAttention(
    ...
    def forward(
        ...
        if self.do_layer_norm_before:
            ...
            #hidden_states = nn.LayerNorm(hidden_states, elementwise_affine=True)
...
class PixelOPTDecoder(OPTDecoder):
    def __init__(self, config):
        super().__init__(config)
        self.layers = nn.ModuleList([PixelOPTDecoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class PixelOPTModel(OPTModel):

    def __init__(self, config):
        super().__init__(config)
        self.decoder = PixelOPTDecoder(config)
...
class PixelOTPForCausalLM(OPTForCausalLM):
    config_class = PixelOPTConfig
