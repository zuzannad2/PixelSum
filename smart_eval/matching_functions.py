...
from bleurt import score as bleurt_score # installed from source: https://github.com/google-research/bleurt
...
class ChrfMatchingFunction(MatchingFunction):
  ...
  # NOTE needed to fix https://github.com/mjpost/sacrebleu/issues/121: 'refs' must be a list of strings
  # Also, the sacrebleu.sentence_chrf() function returns a sacrebleu.Score object, not a float, so we need to call .score on it
  def __call__(
      self,
      reference,
      candidate,
    ...
    if isinstance(reference, list):
      return [
          sacrebleu.sentence_chrf(c, [r]).score for r, c in zip(reference, candidate)] 
    else:
      return sacrebleu.sentence_chrf(candidate, [reference]).score
