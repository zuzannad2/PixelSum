...
# NOTE eventually pull updates from "run_finetuning.py" 
...
def main():  
    ...
    # def string_to_ngrams(s:str, n:int=2) -> list: # NOTE moved to the renderer. Remove later
    #     """
    ...
    def preprocess_examples(batch):
        documents, summaries, num_patches = batch['text'], batch['target'], batch['num_text_patches']
        ...
        for document, summary, n_patch in zip(documents, summaries, num_patches):
            # NOTE add word splitter from Stanza for Chinese (and other languages?)
            # document = document.replace('\n','')
            # encoding = renderer(document)

            # image = encoding.pixel_values
            # n_patch = encoding.num_text_patches
            pixel_values = transforms(Image.open(io.BytesIO(document['bytes'])))
            attention_mask = get_attention_mask(n_patch, seq_length=data_args.max_seq_length)
    ...
    # dataset = load_dataset("Team-PIXEL/PIXELSum_en_wiki_for_TA", keep_in_memory=False, split="train")
    # dataset = load_from_disk("/scratch/project/dd-23-53/zuz/PixelSum/wiki_data/preprocessed_mWiki_for_TA_en")
    # dataset = dataset.filter(lambda x: bool(x['text']) and bool(x['target']))
    
    # # *** mWiki dataset ***
    en = load_dataset("Team-PIXEL/PIXELSum_en_wiki_for_TA", keep_in_memory=False, split="train")
    hi = load_dataset("Team-PIXEL/PIXELSum_hi_wiki_for_TA", keep_in_memory=False, split="train")
    uk = load_dataset("Team-PIXEL/PIXELSum_uk_wiki_for_TA", keep_in_memory=False, split="train")
    zh = load_dataset("Team-PIXEL/PIXELSum_zh_wiki_for_TA", keep_in_memory=False, split="train")
    # en = load_from_disk("/scratch/project/dd-23-53/zuz/PixelSum/wiki_data/preprocessed_mWiki_for_TA_en")
    # hi = load_from_disk("/scratch/project/dd-23-53/zuz/PixelSum/wiki_data/preprocessed_mWiki_for_TA_hi")
    # uk = load_from_disk("/scratch/project/dd-23-53/zuz/PixelSum/wiki_data/preprocessed_mWiki_for_TA_uk")
    # zh = load_from_disk("/scratch/project/dd-23-53/zuz/PixelSum/wiki_data/preprocessed_mWiki_for_TA_zh")
    # Filter 
    # en = en.filter(lambda x: bool(x['text']) and bool(x['target']))
    # hi = hi.filter(lambda x: bool(x['text']) and bool(x['target']))
    # uk = uk.filter(lambda x: bool(x['text']) and bool(x['target']))
    # zh = zh.filter(lambda x: bool(x['text']) and bool(x['target']))
    # Interleave datasets w/ temperature sampling (alpha=0.3)
    dataset = interleave_datasets([en, hi, uk, zh], probabilities=[0.432, 0.123, 0.237, 0.208], 
                                  stopping_strategy="first_exhausted")
    
    if training_args.do_train:
        # train_dataset = dataset['train']
        train_dataset = dataset.shuffle(seed=training_args.seed)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # logger.info(f"After broadcasting: {smart_eval_chrf_out=}; {smart_eval_bleurt_out=}")
      
        # # Save predictions to csv incl. timestamp
