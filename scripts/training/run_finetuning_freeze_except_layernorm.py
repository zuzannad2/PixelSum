...
# assert len(logical_devices) == 0, "Zero GPUs should be visible"
...
from smart_eval.scorer import SmartScorer
from smart_eval.matching_functions import ChrfMatchingFunction
...
def get_model_and_config(model_args: argparse.Namespace): 
    ...
    if "opt" in model_args.decoder_name or "xglm" in model_args.decoder_name or "bloom" in model_args.decoder_name:
        if not model_args.train_decoder:
            for name, param in model.decoder.named_parameters():
                ...
                # NOTE we also freeze the cross-attention layers
                param.requires_grad_(False)
    ...
    for l in list(range(12)):
        model.encoder.encoder.layer[l].layernorm_before.requires_grad_(True)
        model.encoder.encoder.layer[l].layernorm_after.requires_grad_(True)
...
def main():  
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        for pred, label, src in zip(predictions, labels, copy_val_dataset_text):
            smart_eval_chrf_list.append(chrf_scorer.smart_score(
                candidate=pred, 
                reference=label,
                source=src)['smartL']['fmeasure']) # We only care about smartL
        
        smart_eval_chrf_out = float(np.mean(smart_eval_chrf_list))
        ...
        if data_args.log_predictions:
            if rank == 0: # To avoid potential duplicates with distributed workloads
                ...
                # Save scores as json incl. timestamp
                scores = {
                ...
                scores_out = {datetime.now().strftime('%d-%m-%Y_%H-%M-%S'): scores}
                out_file = os.path.join(training_args.output_dir, f"scores_{datetime.now().strftime('%d-%m-%Y_%H-%M-%S')}.json")
